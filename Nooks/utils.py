...
class NooksAllocation:
    ...
    def __init__(self, db, alpha=2):
        self.db = db 
        self.member_vectors = {member["user_id"]: np.array(member["member_vector"]) for member in self.db.member_vectors.find()}
        self.member_dict = {}
        self.member_ids = {}
    ...
    def _update_interacted(self, member_allocs, nooks_allocs):
        for member in range(self.total_members):
            ...
            self.db.temporal_interacted.update_one(
                ...
                {"$set":{"counts": self.temporal_interacted[member].tolist()}},  upsert=True)
            self.db.all_interacted.update_one(
                {"user_id": self.member_ids[member]},
                {"$set":{"counts": self.all_interacted[member].tolist()}},  upsert=True)
    ...
    def create_nook_allocs(self, nooks):
        ...
        for i in range(self.num_iters):
            ...
            for member in all_members_permute:
                ...
                member_allocs[member] = selected_nook
                nooks_mem_int_cnt[selected_nook] += (self.temporal_interacted[member] >= 1)
        ...
        for nook_id in range(len(nooks_allocs)):
            ...
            #.tolist()
            right_swipe_mems_ids = [self.member_ids[member] for member in right_swipe_mems]
            allocations[nooks[nook_id]["_id"]] = ",".join(right_swipe_mems_ids)
            self.db.stories.update_one({"_id": nooks[nook_id]["_id"]}, {
                "$set":{
                    "members": right_swipe_mems_ids
