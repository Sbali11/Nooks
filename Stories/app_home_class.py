...
class AppHome:
    def __init__(self, max_stories):
        self.max_stories = max_stories
        self.all_stories = {}

    def update_stories(self):
        self.all_stories = list(db.user_stories.find({"user_id": user_id, "status": "active"}))
    ...
    def update_home_tab(self, client, event, logger):
        user_id = event["user"]
        if not self.all_stories:
            self.default_message(client, event)
            return

        for stories_row in self.all_stories:
...
app_home = AppHome(max_stories=MAX_STORIES_GLOBAL)
